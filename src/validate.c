...
State_Type check_process(Service_T s) {
        ...
        int64_t uptimeMilli = (int64_t)(s->inf->priv.process.uptime) * 1000LL;
...
State_Type check_program(Service_T s) {
                        ...
                        if (status->operator == Operator_Changed) {
                                if (status->initialized) {
                                        if (Util_evalQExpression(status->operator, s->program->exitStatus, status->return_value)) {
                                                Event_post(s, Event_Status, seauid_status, State_Changed, status->action, "status changed (%d -> %d) -- %s", status->return_value, s->program->exitStatus, output);
                                                status->return_value = s->program->exitStatus;
                                        } else {
                                                Event_post(s, Event_Status, seauid_status, State_ChangedNot, status->action, "status didn't change (%d) -- %s", s->program->exitStatus, output);
                                        }
                                } else {
                                ...
                                if (Util_evalQExpression(status->operator, s->program->exitStatus, status->return_value)) {
                                        rv = State_Failed;
                                        Event_post(s, Event_Status, seauid_status, State_Failed, status->action, "'%s' failed with exit status (%d) -- %s", s->path, s->program->exitStatus, output);
                                } else {
                                        Event_post(s, Event_Status, seauid_status, State_Succeeded, status->action, "status succeeded [status=%d] -- %s", s->program->exitStatus, output);
                                }
                        }
                }
        ...
        if (s->mode & Monitor_Forced || (s->monitor != Monitor_Not && ! _checkSkip(s))) { // The status evaluation may disable service monitoring
                // Start program

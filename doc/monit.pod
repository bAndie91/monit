...
Syntax:
  ...
  IF FAILED PING[4|6]
     ...
     [ADDRESS string]
     [TTL hops]
     [EXPECT [ECHO_REPLY | TTL_EXCEEDED]]
  THEN action
...
TTL <hops> sets the TTL in IP packets. Default is 255.

Default EXPECT is ECHO_REPLY which is an ordinary pong response. Set
TTL_EXCEEDED with low TTL if you want to check the intermediate path
without checking the host itself.
